...
from app.config.settings_sections import (
    BotMixin,
    CoreMixin,
    McpMixin,
    SharedSettings,
...
class Settings(BotMixin, CoreMixin, WorkerMixin, McpMixin, SharedSettings):
    ...
    ``app.config.settings_sections`` (``BotSettings``, ``CoreSettings``,
    ``WorkerSettings``, ``McpSettings``), which share the same base and env
    semantics. Keeping this aggregate means every existing
    ``from app.config import settings`` import keeps working unchanged.
    """
