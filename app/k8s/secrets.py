...
SSH_SECRET_NAME = "vanessa-obsidian-ssh"
...
def rewrite_broker_host(url: str, host: str) -> str:
    parsed = urlparse(url)
...
def build_ssh_secret(*, namespace: str, ssh_dir: Path) -> dict:
    if not ssh_dir.is_dir():
        raise FileNotFoundError(f"ssh directory not found: {ssh_dir}")
    string_data: dict[str, str] = {}
    for name in SSH_FILENAMES:
        path = ssh_dir / name
        if path.is_file():
            string_data[name] = path.read_text(encoding="utf-8")
    if not string_data:
        raise FileNotFoundError(
            f"no ssh key files in {ssh_dir} (looked for {', '.join(SSH_FILENAMES)})"
        )
    return {
        "apiVersion": "v1",
        "kind": "Secret",
