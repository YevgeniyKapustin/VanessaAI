...
def test_cli_apply_ssh_and_namespace(
    tmp_path: Path,
    ...
    assert "name: vanessa-obsidian-ssh" in seen[2]
    assert "PRIVATE" in seen[2]
...
def test_cli_require_ssh_without_dir(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
):
    env = _write_env(tmp_path / ".env")
    monkeypatch.setattr("scripts.k8s_secrets._run_kubectl", lambda *a, **k: None)
    with pytest.raises(SystemExit, match="ssh directory not found"):
        main(
            [
                "--from-env",
                str(env),
